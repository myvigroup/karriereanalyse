...
UNION ALL SELECT 'Analyse Sessions', count(*) FROM analysis_sessions
UNION ALL SELECT 'Analyse Results', count(*) FROM analysis_results
UNION ALL SELECT 'Profile', count(*) FROM profiles;

-- =============================================
-- FIX: Kurs-Daten einfügen (falls leer)
-- =============================================
-- ACHTUNG: Nur ausführen wenn die Diagnose 0 Kurse/Module/Lessons zeigt!
...
-- Bestehende E-Learning Daten löschen (Idempotent — kann mehrfach ausgeführt werden)
DELETE FROM lesson_progress WHERE lesson_id IN (SELECT id FROM lessons WHERE module_id IN (SELECT id FROM modules WHERE course_id IN (SELECT id FROM courses WHERE id LIKE 'c1000000%')));
DELETE FROM lessons WHERE module_id IN (SELECT id FROM modules WHERE course_id IN (SELECT id FROM courses WHERE id LIKE 'c1000000%'));
DELETE FROM modules WHERE course_id IN (SELECT id FROM courses WHERE id LIKE 'c1000000%');
DELETE FROM courses WHERE id LIKE 'c1000000%';
...
-- =============================================
-- VERIFICATION: Prüfe ob alles da ist
-- =============================================
SELECT 'Kurse' AS check_name, count(*) AS result FROM courses WHERE id LIKE 'c1000000%'
UNION ALL SELECT 'Module', count(*) FROM modules WHERE course_id LIKE 'c1000000%'
UNION ALL SELECT 'Lessons', count(*) FROM lessons WHERE module_id IN (SELECT id FROM modules WHERE course_id LIKE 'c1000000%')
UNION ALL SELECT 'Alle published?', count(*) FROM courses WHERE id LIKE 'c1000000%' AND is_published = true;
